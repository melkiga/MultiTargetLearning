...
% Enjoy life!!!
    ...
    ranks = num2cell(ranks);
    ranks{1} = num2str(ranks{1}, format);

    if isnumeric(matrix)
        matrix = num2cell(matrix);
        maxes = num2cell(maxes);
        for h=1:height
            maxes{h} = num2str(maxes{h}, format);
            for w=1:width
                if(~isempty(format))
    ...
    fprintf(fid, '\\begin{threeparttable}\n\\resizebox{\\textwidth}{!}{\\colorbox{yellow!20}{');
    ...
    for h=1:height
        if(~isempty(rowLabels))
            if(strncmp(rowLabels{h},'Average',7) == 1)
                fprintf(fid, '\\hline\n');
            end
            fprintf(fid, '%s &', rowLabels{h});
        end
        if(strncmp(rowLabels{h},'Ranks',5) == 1)
            for w=1:width
                if(strncmp(matrix{h, w},ranks{1},6) == 1)
            ...
            for w=1:width
                if(strncmp(matrix{h, w},maxes{h},6) == 1)
                    fprintf(fid, '\\textbf{%s} &', matrix{h, w});
                else
                    fprintf(fid, '%s &', matrix{h, w});
                end
